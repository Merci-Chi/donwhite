...
searchForm?.addEventListener('submit', event => {
  ...
  if (!searchableCards.length) {
    window.location.href = `services-v1.html?search=${encodeURIComponent(term)}`;
    return;
  }
...
// Run a search passed in from another page.
(() => {
  const term = new URLSearchParams(window.location.search).get('search')?.trim().toLowerCase();
  if (!term || !searchableCards.length) return;
